...
export function NumberField({ value, onChange, decimal = true, nullable = false, className = '', ...rest }: any) {
...
export const TextField = forwardRef<HTMLInputElement, any>(function TextField({ className = '', ...rest }: any, ref) {
...
export function SearchField({ value, onChange, onClear, ...rest }: any) {
  return (
...
export function Segmented({ options, value, onChange, className = '' }: any) {
  ...
  return (
    <div className={`${uiClasses.segmented} seg ${className}`} style={{ '--n': options.length, '--i': i } as any}>
...
export function Stepper({ value, step = 1, onChange, decimal = true, className = '', label, unit }: any) {
...
export function Slider({ value, min = 0, max = 100, step = 1, onChange, className = '' }: any) {
...
export function Check({ checked, onChange, className = '', size }: any) {
  return (
    <button
      role="checkbox"
      aria-checked={!!checked}
...
export function Section({ title, footer, children, className = '' }: any) {
  return (
...
export function Row({ icon, iconTint, title, subtitle, value, accessory = 'none', onClick, danger, children, className = '' }: any) {
  const Tag = onClick ? 'button' : 'div'
  return (
    <Tag className={`${uiClasses.listRow} lrow${onClick ? ' tap' : ''}${danger ? ' danger' : ''} ${className}`} onClick={onClick}>
      {icon && <span className={`${uiClasses.listIcon} lrow-i`} style={iconTint ? { '--tint': iconTint } as any : undefined}><Icon name={icon} /></span>}
...
export function SelectRow({ icon, iconTint, title, value, options, onChange, sheetTitle }: any) {
...
export function Button({ variant = 'plain', size, icon, trailingIcon, children, className = '', ...rest }: any) {
  return (
