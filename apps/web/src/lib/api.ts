...
export async function api<T = any>(path: string, opts: RequestInit = {}): Promise<T> {
  ...
  if (!r.ok) {
    const e = Object.assign(new Error(data.error || ('HTTP ' + r.status)), { status: r.status })
    throw e
  }
  return data
}

const bufToB64u = (buf: ArrayBuffer) => btoa(String.fromCharCode(...new Uint8Array(buf))).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '')
const b64uToBuf = (s: string) => Uint8Array.from(atob(s.replace(/-/g, '+').replace(/_/g, '/')), c => c.charCodeAt(0)).buffer

function toCreationOptions(o: any): PublicKeyCredentialCreationOptions {
  o.challenge = b64uToBuf(o.challenge)
  o.user.id = b64uToBuf(o.user.id)
  ;(o.excludeCredentials || []).forEach(c => { c.id = b64uToBuf(c.id) })
  return o
}
function toRequestOptions(o: any): PublicKeyCredentialRequestOptions {
  o.challenge = b64uToBuf(o.challenge)
...
export async function passkeyRegister(name: string, code = '') {
