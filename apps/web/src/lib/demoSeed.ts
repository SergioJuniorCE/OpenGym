// The example profile behind the demo build (see demo.ts). Imported dynamically, so it stays
...
// switched to RIR. A set is never rewritten (see history.ts), so the stats have to average a
...
export function buildDemoState() {
  ...
  const exWeights: Record<string, any> = {}
  ...
  for (let d = new Date(start); d <= today; d.setDate(d.getDate() + 1)) {
    ...
    const entries = routine.ex.map((cfg, exIdx) => {
      ...
      for (let i = 0; i < cfg.sets; i++) {
        ...
        const s: any = { w, r: Math.max(4, cfg.reps - drop), done: true }
