...
import { EXDB, EXIDX } from './exercises'
...
export function parseWorkoutCSV(text, { unit = 'kg' } = {}) {
  ...
  const resolved = new Map<string, string | null>() // exercise name -> dataset id | null, resolved once
...
export function parseBodyweight(text, { unit = 'kg' } = {}) {
  const s = String(text)
  const out = new Map<string, { w: number, t: number | null }>() // iso date -> { w, t } (one weigh-in per day, the last)
  let fileUnit = ''
...
export function parseImport(text: any, opts?: any) {
  const s = String(text)
