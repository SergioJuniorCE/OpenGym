...
let requests: number, released: number, live: any, listeners: Set<() => void>, reject: boolean
...
const background = async () => { (globalThis.document as any).visibilityState = 'hidden'; if (live) await live.release(); fire(); await settle() }
const foreground = async () => { (globalThis.document as any).visibilityState = 'visible'; fire(); await settle() }
...
beforeEach(async () => {
  ...
  ;({ requestWakeLock, releaseWakeLock, wakeLockSupported } = await import('./wakelock'))
})
