...
import { EXDB, EXIDX, BODYPARTS, isCardio, isBodyweightEq, allExercises, equipmentOf } from './lib/exercises'
import { fmtDate, fmtNum, fmtVol, fmtDur, durPart, todayISO, uid, exCount, DAYN, MONTHS_LONG, ACCENTS } from './lib/format'
import { lastEntryFor, bestWeightFor, buildSets, effectiveRoutineId, workoutVolume, setsDone, setsDoneActive, lastBW, supersetUnits, unitOf, setLabel, defaultConfig, cleanupSg, modeOf, effortOf, isBw, isPerSide, sideReps } from './lib/history'
import { beep, vibrate } from './lib/sound'
import { t, instrFor, getLang, INSTR_LANGS } from './lib/i18n'
import { nav } from './lib/nav'
import { starterRoutines } from './lib/starter'
import Media, { Thumb } from './components/Media'
import Stepper from './components/Stepper'
import Icon from './components/Icon'
import { Button, Slider, Switch, Segmented, SelectRow, Row } from './components/ui'
import { glyphOf, GLYPH_GROUPS, DEFAULT_GLYPH } from './lib/glyphs'
import BodyMap from './components/BodyMap'
import { loadOfWorkouts } from './lib/muscles'
import { parseImport, mergeImport } from './lib/import-csv'
...
function ConfirmDialog({ title, message, confirmText, cancelText, danger, onConfirm, close }: any) {
...
function WeightInput({ value, setValue, unit }: any) {
  const W_HI = wHi(unit)
...
/* ============================ body weight ============================ */
function BwSheet({ required, onDone, close }: any) {
...
function ImportSummary({ parsed, close }: any) {
  const st = useStore(s => s.S)
...
export function importFromApp(file: any, onDone?: any) {
...
function ExerciseDetail({ ex, close }: any) {
  const st = useStore(s => s.S)
...
/* ============================ add to routine ============================ */
function AddToRoutine({ ex, close }: any) {
  const st = useStore(s => s.S)
  const pick = rid => {
    close()
...
function CustomExForm({ existing, prefill, onDone, close }: any) {
...
export const customExSheet = (existing: any, onDone?: any, prefill?: any) => ui().openSheet(close => <CustomExForm existing={existing} prefill={prefill} onDone={onDone} close={close} />)

export function deleteCustomEx(ex: any, afterDelete?: any) {
...
function ExercisePicker({ onPick, close }: any) {
  const st = useStore(s => s.S)
...
function ProgressionFields({ ex, mode, c, setC, routine, unit }: any) {
...
function ExConfig({ ex, existing, onSave, onDelete, close, routine }: any) {
  const st = useStore(s => s.S)
  ...
  const save = () => {
    ...
    if (cardio) onSave({ sets, min: Math.max(1, Math.round(c.min) || 20), speed: Math.max(0, c.speed || 8) })
      ...
      const out: any = { sets, mode: 'reps', reps, weight: Math.max(0, c.weight || 0), ...flags, ...(perSide ? { side: true } : {}), ...prog }
...
export const exConfigSheet = (ex: any, existing?: any, onSave?: any, onDelete?: any, routine?: any) => ui().openSheet(close => <ExConfig ex={ex} existing={existing} onSave={onSave} onDelete={onDelete} routine={routine} close={close} />)
...
function PlanImport({ bundle, close }: any) {
...
/* ============================ workout detail ============================ */
function WorkoutDetail({ w, close }: any) {
  const st = useStore(s => s.S)
  return <>
    <h3>{w.name}</h3>
...
/* ============================ calendar ============================ */
function Calendar({ start, close }: any) {
  const st = useStore(s => s.S)
...
export const calendarSheet = (start?: any) => ui().openSheet(close => <Calendar start={start} close={close} />)

/* shared small workout row (used in lists) */
export function WorkoutRow({ w, onClick }: any) {
  const st = useStore(s => s.S)
...
/* ============================ workout lifecycle ============================ */
export function startFlow(routineId?: any) {
...
function TopWeight({ entryIdx, close }: any) {
  const st = useStore(s => s.S)
  const A = st.active
...
function WorkoutComplete({ close }: any) {
...
function FinishSummary({ w, prs, e1prs = [], close }: any) {
  const st = useStore(s => s.S)
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
