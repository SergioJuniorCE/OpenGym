...
sw.addEventListener('push', e => {
  ...
  e.waitUntil(sw.registration.showNotification(data.title || 'openGym', {
    body: data.body || '',
    icon: 'icon-512.png',
    badge: 'icon-180.png',
...
sw.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(sw.clients.matchAll({ type: 'window' }).then(clients => {
    const c = clients.find(c => 'focus' in c)
    return c ? c.focus() : sw.clients.openWindow('./')
  }))
})
