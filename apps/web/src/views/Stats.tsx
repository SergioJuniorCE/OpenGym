...
import { lastBW, streakWeeks, setLabel, modeOf, effortOf } from '../lib/history'
import { fmtNum, fmtDate, fmtVol, todayISO, weekKey } from '../lib/format'
import { t } from '../lib/i18n'
import { bwSheet, goalSheet, calendarSheet, workoutDetailSheet, WorkoutRow, bwDeltaColor } from '../sheets'
...
import { Button, Segmented, SelectRow } from '../components/ui'

// Which muscles the training in a window actually hit — and, the point of the card,
// which ones it keeps missing. Shading is relative within the window (lib/muscles.ts).
function MuscleBalance({ S }: any) {
...
// is computed in RIR (lib/effort.ts) and converted to whichever scale this profile reads.
...
export default function Stats() {
  ...
  const exHist = [...new Set<string>(S.workouts.flatMap((w: any) => w.entries.map((e: any) => e.id)))].filter(id => EXIDX[id]).sort((a, b) => EXIDX[a].n < EXIDX[b].n ? -1 : 1)
