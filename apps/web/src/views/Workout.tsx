...
import { effectiveRoutine, lastEntryFor, bestWeightFor, buildSets, setsDoneActive, supersetUnits, unitOf, setLabel, modeOf, isBw, isPerSide, sideReps, repStep, EFFORT, effortOf, stepEffort, capEffort } from '../lib/history'
import { fmtNum, fmtDate, todayISO, exCount, DAYN } from '../lib/format'
import { beep, vibrate } from '../lib/sound'
import { t } from '../lib/i18n'
import { api } from '../lib/api'
import Media from '../components/Media'
import { startFlow, exercisePicker, exConfigSheet, exerciseDetailSheet, topWeightSheet, finishWorkout, workoutCompleteSheet, confirmSheet } from '../sheets'
import Icon from '../components/Icon'
import { Button, Check, NumberField } from '../components/ui'
...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onAddSet, onRemoveSet, onStartTimed }: any) {
  const S = useStore(s => s.S)
  ...
  return <>
    ...
    {plan && plan.why && plan.kind !== 'off' && <div className={'progline' + (plan.kind === 'deload' ? ' warn' : '')}>
      ...
      <span>{t(plan.why[0], ...plan.why.slice(1))}</span>
    </div>}
